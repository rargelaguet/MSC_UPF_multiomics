...
```{r}
file <- "/Users/ricard/Google Drive/Teaching/Data_integration_UPF_June2019/pca_vignette/lenna/lena.png"
Y <- readPNG(file) %>% .[100:400,]
```
...
```{r}
filename <- "/Users/ricard/Google Drive/Teaching/Data_integration_UPF_June2019/pca_vignette/lenna/lenna_reconstructed.png"
writePNG(Ypred, filename)
```
...
### **compare the number of values that the original matrix has versus the number of values that are stored in the PCA representation** 
...
### **(Q) Try to do compression with 50 principal components. Use the `prcomp` function. Does the reconstructed image look better?**  
