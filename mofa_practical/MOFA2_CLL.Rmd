...
This tutorial demonstrates how to use MOFA on the bulk multi-omics data set that was used in the [first publication of MOFA](https://msb.embopress.org/cgi/doi/10.15252/msb.20178124).  
It corresponds to the original [MOFA v1 vignette](https://bioconductor.org/packages/release/bioc/vignettes/MOFA/inst/doc/MOFA_example_CLL.html) upgraded to MOFA2 code.  

Briefly, the data set consists of N=200 samples from Chronic Lymphocytic Leukemia (CLL) patients where four omics were profiled from blood samples: DNA methylation (450K Illumina microarray), bulk RNA-seq, somatic mutations and drug response data from blood for N=200 patients with. The data set was introduced in detail [in this article](https://www.jci.org/articles/view/93801) and can be downloaded [here](http://bioconductor.org/packages/release/data/experiment/html/BloodCancerMultiOmics2017.html)


# Load libraries

Make sure that you have installed the [MOFA2](https://github.com/bioFAM/MOFA2#installation) and the [MOFAdata](https://www.bioconductor.org/packages/release/data/experiment/html/MOFAdata.html) package. See the [installation instructions]() if you have questions
```{r, message=FALSE}
library(knitr)
library(MOFA2)
library(MOFAdata)
...
As shown below, the mean is fairly uncorrelated from the variance, which is a requirement for the use of a Gaussian likelihood model in most statistical models
```{r}
to.plot <- data.table(
...
DNA methylation is calculated for every CpG site using the [M-value](https://bmcbioinformatics.biomedcentral.com/articles/10.1186/1471-2105-11-587), which provides a better summary statistic for downstream analysis.  
...
In this study they have measured the effect of relevant drugs *ex vivo* using a high-throughput assay. 
...
- **expectations**: expectations of the posterior distributions for the weights (W) and the factors (Z)
...
### Rename mRNA from ENSEMBLE IDs to gene names
...
Keep the model with the ENSEMBL IDs for the [gene set enrichment analysis section](#gene-set-enrichment-analysis)
```{r}
...
- Can you explore the methylation signal for this Factor?
...
<!-- Clinicians were not able to classify patients XXX in a specific disease subgroup using clinical markers. Could you use the molecular data to find the most similar and provide clinical recommendations?  -->
...
To answer this we can first collect the genes with the largest weights for Factor 1. Then we can do boxplots stratifying samples by IGHV status, followed by statiscal testing with the null hypothesis that the average gene expression does not differ between groups (a simple t-test should work).  

Extract mRNA weights from the MOFAobject
```{r}
# Notice abs=TRUE as we do not distinguish between direction of effect
...
It is always advised to not rely only on the p-values and to visualise which genes are driving the enrichment within each pathways. 
```{r}
