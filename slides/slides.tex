...
\begin{document}
	...
	\leavevmode\newline
	\centering
	\includegraphics[height=5cm]{img/Ritchie2015_global_analysis.png}
	\end{frame}
	...
	\leavevmode\newline
	This can be written in a more compact form by defining $\bfS \in \R^{D \times D}$ as the data covariance matrix: $\bfS = \frac{1}{N}\sum_{n=1}^{N} (\bfy_n - \hat{\bfy})(\bfy_n - \hat{\bfy})^T$: %Note that if the data is centered this is equal to $\bfS = \bfY \bfY^T. Thus:
	\[
